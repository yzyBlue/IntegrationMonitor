	...
	public void serverLogTest() {
		try {
			...
			if (loginStatus.getStatus() == Status.SUCCESS) {
				...
				serverLogReceived = (LinkedList<String[]>) mirthClient.invokePluginMethodAsync("Server Log",
		...
		try {
			...
			if (loginStatus.getStatus() == Status.SUCCESS) {
				...
				connectionInfoLogsReceived = (LinkedList<String[]>) mirthClient
						.invokePluginMethodAsync(DASHBOARD_SERVICE_PLUGINPOINT, GET_CONNECTION_INFO_LOGS, null);
	...
	public void getStatisticsTest() throws ClientException {
		...
		if (loginStatus.getStatus() == Status.SUCCESS) {
			...
			dashboardStatusList = serializer.deserializeList(
					mirthClient.getServerConnection().executePostMethodAsync("/channelstatus", params),
		...
		logger.info(com.mirth.connect.donkey.model.message.Status.FILTERED + " : " + dashboardStatusList.get(0)
				.getStatistics().get(com.mirth.connect.donkey.model.message.Status.FILTERED));
		logger.info(com.mirth.connect.donkey.model.message.Status.SENT + " : "
				+ dashboardStatusList.get(0).getStatistics().get(com.mirth.connect.donkey.model.message.Status.SENT));
		logger.info(com.mirth.connect.donkey.model.message.Status.RECEIVED + " : " + dashboardStatusList.get(0)
				.getStatistics().get(com.mirth.connect.donkey.model.message.Status.RECEIVED));
		logger.info(com.mirth.connect.donkey.model.message.Status.ERROR + " : "
				+ dashboardStatusList.get(0).getStatistics().get(com.mirth.connect.donkey.model.message.Status.ERROR));
